...
class ResWorldCustomNuScenesDataset(VADCustomNuScenesDataset):
    def __init__(
        ...
        **kwargs
        ...
        self.osz_root = osz_root
        # token -> info 索引, 用于定位下一帧 (可见性转移自监督)
        self._token2info = None
        if self.osz_root is not None:
            self._token2info = {info['token']: info for info in self.data_infos}
    ...
    def get_data_info(self, index):
        ...
        # OSZ 资产路径: 当前帧与各相邻帧 (LoadOSZAnnotations 消费)
        if self.osz_root is not None:
            input_dict['curr']['osz_path'] = osp.join(
                self.osz_root, info['token'] + '.npz')
            for adj_info in info_adj_list:
                adj_info['osz_path'] = osp.join(
                    self.osz_root, adj_info['token'] + '.npz')
            # 下一帧 (可见性转移自监督目标), 场景末尾为 None
            next_info = None
            if info.get('next', '') != '':
                cand = self._token2info.get(info['next'], None)
                if cand is not None and cand['scene_token'] == info['scene_token']:
                    next_info = cand
                    next_info['osz_path'] = osp.join(
                        self.osz_root, next_info['token'] + '.npz')
            input_dict['next_info'] = next_info
