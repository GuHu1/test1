...
class TokenLearner(nn.Module):
    ...
    def forward(self, inputs, logit_bias=None, deterministic=True):
        """
        Args:
            inputs: Inputs of shape `[B, HW, C]` or `[B, C, H, W]`.
            logit_bias: Optional bias of shape `[B, 1, HW]`, added to the
                attention logits before softmax (e.g. OSZ risk bias).
        ...
        # selected: [B, HW, num_token]
        if logit_bias is not None:
            # [B, 1, HW] -> [B, HW, 1], 广播到所有 token 的 logit
            selected = selected + logit_bias.permute(0, 2, 1)
